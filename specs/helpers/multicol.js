...
import { createFragments } from "../../src/layout.js";
...
export async function multicol(container) {
	...
	const columnGap = style.columnGap === "normal" ? 0 : parseFloat(style.columnGap) || 0;

	const columnWidth = (totalWidth - (columnCount - 1) * columnGap) / columnCount;
	...
	for (let i = 0; i < fragments.length; i++) {
		...
		colEl.appendChild(fragments[i].build(prevBT));
