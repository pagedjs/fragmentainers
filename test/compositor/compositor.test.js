...
test.describe("buildInlineContent", () => {
  ...
  test("skips inline elements whose content is entirely past the visible range", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { buildInlineContent } = await import("/src/compositor/compositor.js");
      const { INLINE_TEXT, INLINE_OPEN_TAG, INLINE_CLOSE_TAG } = await import("/src/core/constants.js");
  ...
  test("skips inline elements whose content is entirely before the visible range", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { buildInlineContent } = await import("/src/compositor/compositor.js");
      const { INLINE_TEXT, INLINE_OPEN_TAG, INLINE_CLOSE_TAG } = await import("/src/core/constants.js");
