...
import { resolveColumnDimensions } from "../src/column-sizing.js";
...
describe("resolveColumnDimensions", () => {
  ...
  it("column-count only, with gap", () => {
    ...
    assert.equal(count, 3);
    assert.equal(width, 200); // (620 - 2*10) / 3
  });
  ...
  it("column-width only — columns expand to fill space", () => {
    ...
    assert.equal(count, 3);
    assert.ok(Math.abs(width - 500 / 3) < 0.01);
  });
  ...
  it("both specified with gap", () => {
    ...
    assert.equal(count, 3);
    assert.ok(Math.abs(width - 610 / 3) < 0.01);
  });
