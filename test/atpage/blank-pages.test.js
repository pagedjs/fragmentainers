...
test.describe("forcedBreakValue on tokens", () => {
  ...
  test("break-before: right stores value through layout", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      return { isForcedBreak: forcedToken.isForcedBreak, forcedBreakValue: forcedToken.forcedBreakValue };
    });
    expect(result.isForcedBreak).toBe(true);
...
test.describe("Blank page insertion", () => {
  test("break-before: left inserts a blank page when content is on a right page", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { createFragments } = await import("/src/core/layout-request.js");
      const { PageResolver } = await import("/src/atpage/page-resolver.js");
      const { blockNode } = await import("/test/fixtures/nodes.js");
  ...
  test("break-before: right inserts a blank page when next page would be left", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { createFragments } = await import("/src/core/layout-request.js");
      const { PageResolver } = await import("/src/atpage/page-resolver.js");
      const { blockNode } = await import("/test/fixtures/nodes.js");
...
test.describe("First page blank page edge case", () => {
  test("first child with break-before: left inserts blank page 0 (page 0 is right)", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { createFragments } = await import("/src/core/layout-request.js");
      const { PageResolver } = await import("/src/atpage/page-resolver.js");
      const { blockNode } = await import("/test/fixtures/nodes.js");
  ...
  test("first child with break-before: right does NOT insert blank (page 0 is right)", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { createFragments } = await import("/src/core/layout-request.js");
      const { PageResolver } = await import("/src/atpage/page-resolver.js");
      const { blockNode } = await import("/test/fixtures/nodes.js");
